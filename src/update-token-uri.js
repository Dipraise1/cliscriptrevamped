 ...
 * @param {string} newUri - The new URI for the token metadata
 * @param {string} walletPath - Path to the wallet keypair file
 * @returns {Promise<Object>} Result of update operation
 */
export async function updateTokenUri(mintAddress, newUri, walletPath) {
  try {
    // Validate inputs
    if (!mintAddress) {
    ...
    if (!newUri) {
      throw new Error('New URI is required');
    }
    ...
    // Update the metadata
    console.log(chalk.blue(`🔄 Updating metadata URI to: ${newUri}`));
    ...
    console.log(chalk.green(`✅ Successfully updated metadata URI!`));
    ...
    console.error(chalk.red(`\n❌ Error updating token URI: ${error.message}`));
    if (error.logs) {
...
if (process.argv[1].endsWith('update-token-uri.js')) {
  const args = process.argv.slice(2);
  
  if (args.length < 2) {
    console.error(chalk.red('Usage: node update-token-uri.js <token_mint_address> <new_uri> [wallet_path]'));
    process.exit(1);
  }
  ...
  updateTokenUri(mintAddress, newUri, walletPath)
    .then(result => {
      if (!result.success) {
        process.exit(1);
      }
